...
macro_rules! bpf_syscall_test {
    ($name:ident, $child_code:block, $expects:expr) => {
        #[test]
        #[ignore]
        fn $name() {
            ...
            let expects: &[(&str, fn(&Event) -> bool)] = &$expects;
            for (name, check) in expects.iter() {
                let found = events.iter().any(|e| check(e));
...
fn test_bpf_captures_signals() {
    ...
    bpf.add_target_pid(std::process::id()).expect("Failed to add parent pid");
...
bpf_syscall_test!(
    test_bpf_captures_misc_basic_syscalls,
    {
        let mut pipefds = [-1, -1];
    ...
    [
        ("Syscall", |e: &Event| matches!(&e.event, Some(Oneof::Syscall(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_getsockopt,
    {
        let fd = libc::socket(libc::AF_INET, libc::SOCK_STREAM, 0);
        if fd >= 0 {
    ...
    [
        ("SocketOpt", |e: &Event| matches!(&e.event, Some(Oneof::SocketOpt(s)) if s.level == libc::SOL_SOCKET && s.optname == libc::SO_TYPE)),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_ioctl,
    {
        let mut pipefds = [-1, -1];
        libc::pipe(pipefds.as_mut_ptr());
        if pipefds[0] >= 0 {
    ...
    [
        ("Ioctl", |e: &Event| matches!(&e.event, Some(Oneof::Ioctl(i)) if i.cmd == libc::FIONREAD as u64)),
    ]
);
...
bpf_syscall_test!(
    test_bpf_captures_memory_syscalls,
    {
    ...
    [
        ("Mmap", |e: &Event| matches!(&e.event, Some(Oneof::Mmap(_)))),
        ("Mprotect", |e: &Event| matches!(&e.event, Some(Oneof::Mprotect(_)))),
        ("Munmap", |e: &Event| matches!(&e.event, Some(Oneof::Munmap(_)))),
        ("Brk", |e: &Event| matches!(&e.event, Some(Oneof::Brk(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_file_io_syscalls,
    {
        let path = b"/dev/null\0".as_ptr() as *const i8;
    ...
    [
        ("FileRead", |e: &Event| matches!(&e.event, Some(Oneof::FileRead(_)))),
        ("FileWrite", |e: &Event| matches!(&e.event, Some(Oneof::FileWrite(_)))),
        ("FileClose", |e: &Event| matches!(&e.event, Some(Oneof::FileClose(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_network_syscalls_extended,
    {
    ...
    [
        ("Bind", |e: &Event| matches!(&e.event, Some(Oneof::Bind(_)))),
        ("Sendto", |e: &Event| matches!(&e.event, Some(Oneof::Sendto(_)))),
        ("Recvfrom", |e: &Event| matches!(&e.event, Some(Oneof::Recvfrom(_)))),
        ("Accept", |e: &Event| matches!(&e.event, Some(Oneof::NetworkAccept(_)) | Some(Oneof::Accept(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_fs_namespace_syscalls,
    {
    ...
    [
        ("Rename", |e: &Event| matches!(&e.event, Some(Oneof::Rename(_)))),
        ("Unlinkat", |e: &Event| matches!(&e.event, Some(Oneof::Unlinkat(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_multiplexing_syscalls,
    {
    ...
    [
        ("Poll", |e: &Event| matches!(&e.event, Some(Oneof::Poll(_)))),
        ("EpollWait", |e: &Event| matches!(&e.event, Some(Oneof::EpollWait(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_process_control_syscalls,
    {
...
bpf_syscall_test!(
    test_bpf_captures_admin_syscalls,
    {
    ...
    [
        ("Bpf", |e: &Event| matches!(&e.event, Some(Oneof::Bpf(_)))),
        ("Capset", |e: &Event| matches!(&e.event, Some(Oneof::Capset(_)))),
    ]
);

bpf_syscall_test!(
    test_bpf_captures_mount_namespace_syscalls,
    {
    ...
    [
        ("Chroot", |e: &Event| matches!(&e.event, Some(Oneof::Chroot(_)))),
        ("PivotRoot", |e: &Event| matches!(&e.event, Some(Oneof::PivotRoot(_)))),
        ("Unshare", |e: &Event| matches!(&e.event, Some(Oneof::Unshare(_)))),
        ("Setns", |e: &Event| matches!(&e.event, Some(Oneof::Setns(_)))),
    ]
);